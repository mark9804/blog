...
export async function getPostsData(config: SiteConfig, ops: RSSOptions) {
  ...
  let posts: PostInfo[] = [];
  const fileContentPromises = pagesData.reduce(
    (prev, f) => {
      const { isDynamic, dynamicRoute, filepath } = f;
      const contentPromise =
        isDynamic && dynamicRoute
          ? Promise.resolve(
              renderDynamicMarkdown(
                filepath,
                dynamicRoute?.params,
    ...
    {} as Record<
      string,
      {
        ...
        datePromise: Promise<Date | undefined> | undefined | Date;
      }
    >
  );
  ...
  for (const page of pagesData) {
    ...
    // 获取封面图
    frontmatter.cover =
      (frontmatter.cover ?? fileContent.match(imageRegex)?.[1]) || "";

    const targetPage = normalizePath(
      page.rewritePath ? page.rewritePath : page.page
    );
    const url = joinPath(config.site.base, targetPage)
      .replace(/(^|\/)index\.md$/, "$1")
  ...
  posts = posts.filter(p => {
    ...
    // 跳过未发布的文章
    if (p.frontmatter.publish === false && !ignorePublish) return false;
  ...
  for (const post of posts) {
    ...
    feed.addItem({
      ...
      image: frontmatter?.cover
        ? new URL(frontmatter?.cover, baseUrl).href
        : "",
      date: new Date(date),
    });
  }

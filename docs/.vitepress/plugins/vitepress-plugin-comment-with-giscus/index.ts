import giscus, { AvailableLanguage, type GiscusProps } from "@giscus/vue";
import { Component, createApp, h, nextTick, onMounted, Ref, watch } from "vue";
import { PageData, Route } from "vitepress";

export type vitepressAPI = {
  frontmatter: Ref<PageData["frontmatter"]>;
  route: Route;
};

export type GiscusPropsType = GiscusProps & {
  lightTheme?: string;
  darkTheme?: string;
...
const setGiscus = (
  ...
  // 首页是否显示评论
  if (
    !props.homePageShowComment &&
    (!location.pathname || location.pathname === "/")
  ) {
    return;
  }
  const isDark: boolean =
    document.querySelector("html")?.className.indexOf("dark") !== -1;
    ...
    mutations.forEach(mutation => {
      if (mutation.type == "attributes") {
        ...
        comment?.setAttribute(
          "theme",
          (element! as HTMLElement).className.indexOf("dark") !== -1
            ? darkTheme
            : lightTheme
        );
      }
 ...
 * @param defaultEnable default enable comment area (默认启用评论区)
 */
export const giscusTalk = (
  props: GiscusPropsType,
  vitepressObj: vitepressAPI,
  defaultEnable: boolean = true
) => {
  onMounted(() => {
